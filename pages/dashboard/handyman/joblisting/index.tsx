...
export default function Index() {
  ...
  const handleUpdate = async (data: any) => {
    try {
      await UpdatedFilter.mutateAsync(data, {
        onSuccess(data) {
          // Optional: Erfolgreiche Aktualisierung hier verarbeiten
        },
      });
    } catch (error) {
      console.error(error);
  ...
  }, [GetFilter]); // Abhängigkeit hinzugefügt

  useEffect(() => {
    const newFilter = { ...filter, categories: selectCard };
    handleUpdate(newFilter);
    setFilter(newFilter);
  }, [selectCard, filter, handleUpdate]); // Abhängigkeiten hinzugefügt
      ...
      <HandymanLayout>
        <main className="pb-20">
          <div className="">
            ...
            <div className="p-4">
              {Object?.values(filter)?.some((i) => i?.length > 0) && isPending ? (
                <Loader />
